    ...
    <div class='SupervisionCenter'>
      ...
      <div class="centerBox">
        <div class="w-4-5 full-height">
          <oneDayTour :data='entApplyData' v-if='!totwoP'></oneDayTour>
          <oneDayDouble v-else></oneDayDouble>
        </div>
        <div class="w-1-5 full-height">
          <Container class="full-width h-3-5" :title="'一日办地图'"></Container>
          <Container class="full-width h-2-5" :title="'数据对接监控'">
             <ChartsBarHorizontal :data="barData" :showLabel='false' 
...
export default {
  ...
  mounted() {
    ...
    // 市场监管成功总数
    // axios.get('/monitor/info/detail?dataTime=2020-07-03 00:00:00&indexCode=applyDone').then(res => {
    ...
    axios.get('/monitor/info/detail?dataTime='+ now +'&indexCode=ssDOne').then(res => {
     ...
     // 税收办理时长
    axios.get('/monitor/info/detail?dataTime=2020-07-03 00:00:00&indexCode=avgssHours').then(res => {
