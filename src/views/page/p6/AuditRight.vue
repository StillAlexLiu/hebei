  ...
  mounted () {
    ...
    axios.get('/monitor/check/getCaseProcessData').then(res => {
      ...
      this.graphData = [
        ...
        {
          name: '立案', index: 1, value: data.fileCase, type: 1
        },
        {
          name: '调查终结', index: 2, value: data.endOfInvestigation, type: 2
        },
        {
          name: '处罚建议', index: 3, value: data.punishmentSuggestions, type: 3
        },
        // {
        //   name: '听证', index: 4, value: data.hearing, type: 'getAlertHearing'
        // },
        {
          name: '处罚决定', index: 4, value: data.penaltyDecision, type: 4
        },
        {
          name: '执行', index: 5, value: data.implement,type: 5
        },
        {
          name: '办案完成', index: 6, value: data.caseCompletion, type: 6
        }
      ]
    })
    // 稽查办案线索来源
