<template>
  <div class="CenterMap full">
    ...
    >
    <div slot="tableList">
      <messgae :data='messData' v-if="tabList"  @close='closeBtn'></messgae>
    </div>
      <div slot="info" :class="{full: routeName==='稽查办案'}">
        ...
        <tablePeople :data='peopleList' v-if="routeName==='综合监管' & peopleList.length > 0"  @data='messageData'></tablePeople>
        <KeepOnRecordMapInfo v-if="routeName==='许可备案'" :data="p10Info" />
                  ...
                  if (this.latPoint[i].address.substring(2, 3) === '省') {
                    if (
                      ...
                      list[l].points.push({
                        ...
                        icon: icon,
                        coordinate: [
                          this.latPoint[i].longitude,
                          this.latPoint[i].latitude
                        ],
                        leafNodePoint: true,
                    ...
                    if (
                      this.latPoint[i].address.substring(0, 2) ===
                      list[l].name.substring(0, 2)
                    ) {
                      list[l].points.push({
                        name: this.latPoint[i].name,
            ...
            for (let i = 0; i < data.length; i++) {
              if (Number(data[i].level) === 1) {
                this.point.push({
                  name: data[i].name,
                  coordinate: [data[i].lng, data[i].lat],
                  // icon: require('../p2/compontes/img/1112.png'),
                  icon: this.icon,
                  address: data[i].name,
                ...
                this.point.push({
                  name: data[i].name,
                  coordinate: [data[i].lng, data[i].lat],
                  // icon: require('../p2/compontes/img/12.png'),
                  icon: this.icon,
                  address: data[i].name,
                  pripId: data[i].adCode,
                  cliType: item.cliType,
                ...
                this.point.push({
                  name: data[i].name,
                  coordinate: [data[i].lng, data[i].lat],
                  // icon: require('../p2/compontes/img/13.png'),
                  icon: this.icon,
                  address: data[i].name,
                  pripId: data[i].adCode,
                  cliType: item.cliType,
        ...
        .then(res => {
          ...
          for (let d = 0; d < data5.length; d++) {
            this.p1Info['注册信息']['股权出质登记信息'].push({
              ...
              出质股权数额: data5[d].IMPAM,
              单位: data5[d].PLEDAMUNIT,
              质权人: data5[d].IMPORG,
              质权人证件号码: data5[d].PLEDBLICNO,
        ...
        .then(res => {
          ...
          for (let i = 0; i < data.length; i++) {
            this.p1Info['注册信息']['股权冻结信息'].push({
              ...
              被执行人: data[i].INV,
              被冻结股权所在市场主体名称: data[i].CORENTNAME,
              股权数额: data[i].FROAM,
        ...
        .then(res => {
          ...
          for (let i = 0; i < data.length; i++) {
            this.p1Info['注册信息']['商标注册信息'].push({
              ...
              商标类别: data[i].INT_CLS,
              注册公告日期: data[i].REG_ANNC_DATE,
              注册公告期号: data[i].REG_ANNC_ISSUE,
              商标专用权起始日期: data[i].PROPERTY_BGN_DATE,
              商标专用权截止日期: data[i].PROPERTY_END_DATE,
